$(function(){
	...
	(function () {
		...
		var update_ul = update_div.find('ul');
		var arrData = [
		...
		var iHeight = update_ul.find('li').height();
		...
		update_div.hover(function(){
			clearInterval(timer)
		}, autoPlay);
		
		function doMove (num) {
			iNow += num;
			if(iNow >= arrData.length) iNow = 0;
			if(iNow < 0) iNow = arrData.length-1;
			update_ul.stop().animate({'top': -iHeight*iNow}, 800);
		}
